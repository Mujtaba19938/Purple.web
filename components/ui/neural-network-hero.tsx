...
function ShaderPlane() {
    ...
    useFrame((state) => {
        ...
        // Camera is at z=1, fov=75 degrees
        const distance = 1;
        const fov = 75;
        ...
        // Scale the plane to cover the entire viewport with extra margin to prevent cutoff
        // Using 1.5x scale to ensure full coverage even with aspect ratio changes
        meshRef.current.scale.set(planeWidth * 1.5, planeHeight * 1.5, 1);
    });
...
function ShaderBackground() {
    ...
    return (
        <div
            ref={canvasRef}
            className="absolute inset-0 z-0 w-full h-full overflow-hidden transition-colors duration-300 bg-white dark:bg-black pointer-events-none"
            aria-hidden
        >
            <Canvas
                camera={camera}
            ...
            <div className="pointer-events-none absolute inset-0 bg-gradient-to-t from-black/30 via-transparent to-black/20 dark:from-black/30 dark:via-transparent dark:to-black/20" />
        </div>
    );
}
    ...
    return (
        <section ref={sectionRef} className="relative min-h-screen w-screen overflow-hidden">
            ...
            <div className="relative z-10">
                <div className="mx-auto flex max-w-7xl flex-col items-start gap-6 px-6 pb-24 pt-36 sm:gap-8 sm:pt-44 md:px-10 lg:px-16">
                    <div className="absolute top-6 right-6 z-20">
                        <SlideTabs />
                    </div>

                    <h1 ref={headerRef} className="max-w-2xl text-left text-5xl font-extralight leading-[1.05] tracking-tight text-black sm:text-6xl md:text-7xl dark:text-white">
                        {title}
                    </h1>

                    <p ref={paraRef} className="max-w-xl text-left text-base font-light leading-relaxed tracking-tight text-black/75 sm:text-lg dark:text-white/75">
                        {description}
                    </p>
                    ...
                    <div ref={ctaRef} className="flex flex-wrap items-center gap-3 pt-2">
                        {ctaButtons.map((button, index) => (
                            <a
                                key={index}
                                href={button.href}
                                className={`rounded-full border border-black/20 px-7 py-3 text-sm font-medium tracking-tight transition-all focus:outline-none focus:ring-2 focus:ring-black/40 duration-300 transform active:translate-y-[1px] dark:border-white/20 dark:focus:ring-white/40 ${button.primary
                                        ? "bg-black text-white shadow-[0_18px_45px_rgba(0,0,0,0.3)] hover:bg-black/90 hover:shadow-[0_24px_60px_rgba(0,0,0,0.4)] dark:bg-white dark:text-black dark:shadow-[0_18px_45px_rgba(0,0,0,0.8)] dark:hover:bg-white/90 dark:hover:shadow-[0_24px_60px_rgba(0,0,0,0.9)]"
